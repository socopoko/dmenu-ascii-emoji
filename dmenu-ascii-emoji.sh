...
declare -A key_value_pairs=(
    ...
    ["donger"]="ヽ༼ຈل͜ຈ༽ﾉ"
    ["dontcare,idc"]="(- ʖ̯-)"
    ["donotwant,do"]="not want,dontwant ヽ(｀Д´)ﾉ"
    ["dope"]="<(^_^)>"
    ["doublequotesleft,<<"]="«"
    ["doublequotesright,>>"]="»"
    ["doubleflat"]="𝄫"
...
# Create a list of keys for dmenu
keys=$(printf "%s\n" "${!key_value_pairs[@]}")

# Prompt the user to select a key using dmenu
selected_key=$(echo -e "$keys" | dmenu -p "asciimoji:")

# Check if the selected key is valid
if [[ -n "${key_value_pairs[$selected_key]}" ]]; then
    # Copy the corresponding value to the clipboard
    echo -n "${key_value_pairs[$selected_key]}" | xclip -selection clipboard
    echo "Value copied to clipboard: ${key_value_pairs[$selected_key]}"
else
    echo "Invalid key selected: $selected_key"
fi
